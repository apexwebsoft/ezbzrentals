...
from rentals.models import AccountSetting, Activity, Amenities, AmenitiesType, Attributes, BasicRates, Bed, Bookings, \
    Channel, ChannelManagement, CompanyProfile, Country, CustomServices, Discount, Currency, DiscountType, \
    EarlyBirdDiscount, ExtraServices, Feedback, HouseRules, Invoice, InvoiceItem, LongStayDiscount, NearByAmenities, \
    Notification, OtherRooms, PropertyRole, Rate, Ratetype, Rental, RentalAmenities, RentalBasic, RentalBed, \
    RentalBookingRules, RentalCleaning, RentalDeposit, RentalInstruction, RentalLocation, RentalNearByAmenities, \
    RentalOtherRooms, RentalPolicy, RentalRoom, RentalTax, RentalsGallery, Rentaltype, Policy, Room, Roomtype, \
    SeasonalRates, Subscription, SubscriptionPlan, Tax, Taxtype, UserProfile
...
def signup(request):
    if request.method == "POST":
        ...
        if LastRow.is_staff==0 and LastRow.is_superuser== 0:
            ...
            UserProfiles.language_spoken = 'NA' 
...
def rental_select_channels(request,id):
    ...
    request_booking_enabled = Channel.objects.filter(request_booking=1)
    context={
        'channels':channels,
        'rental_id':id,
        'rentalchannel':rentalchannel,
        'channeltype':rentalchannel.channels,
        'request':rentalchannel.request_booking,
        'request_booking_enabled': request_booking_enabled
    }
...
def rental_channel_update(request,id):
    if request.method == "POST":
        ...
        rentalchannel.request_booking=request.POST.getlist('request_booking[]')
        rentalchannel.save()
        messages.success(request, 'Row added Successfully.')
    return redirect(request.META.get('HTTP_REFERER')) 
...
def profile(request):
    ...
    years = [year for year in range(current_year-100, current_year-18+1)]
    dates = []
    for year in years:
        date = datetime.date(year, 1, 1)
        date_string = date.strftime('%d/%m/%Y')
        dates.append((date, date_string))
...
def demo_api(request):
    ...
    url = 'https://apidemo.mybookingpal.com/product?jwt=jwt'
    

    headers = {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'X-API-key':'IJL9jd7BRYrE7oKkSmUuL3sy2MlCuIsxORwuZ7B7'
    }
...
def account_setting_update(request):
    user=User()
    try:
        account = AccountSetting.objects.get(user_id=id)
        created = False
    except AccountSetting.DoesNotExist:
        account = AccountSetting(user_id=id)
        created = True
...
def account_setting(request):
    ...
    try:
        ...
        account = AccountSetting(user_id=request.user.id)
        created = True

    if request.method == 'POST':
        account.account_type =  request.POST.get('account_type')
        account.account_email_address =  request.POST.get('account_email_address')
        account.prefered_currency =  request.POST.get('prefered_currency')
        account.selected_plan =  request.POST.get('selected_plan')
        account.save()
        messages.success(request, ' Data added Successfully.')
    ...
    return render(request, 'users/user/account-setting.html', {'account': account})


def notification(request):
    
    try:
        notification = Notification.objects.get(user_id=request.user.id)
        created = False
    except Notification.DoesNotExist:
        notification = Notification(user_id=request.user.id)
        created = True
    ...
    return render(request, 'users/user/notification.html', {'notification': notification})
